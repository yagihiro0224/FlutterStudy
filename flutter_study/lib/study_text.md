...
3. 실행 클래스 이해하기 : lib/main.dart
   ...
   상속을 하게 되면 에러가 뜨는데 이는 빌드 메소드를 구현(오버라이드) 하지 않아서 이다.
   빌스 메소드는 너의 위젝의 UI를 만들어 주는 거임.
   ...
   오리지널 스타일 어플을 만든다고 하더라도 선택을 해줘야 됨.
...
5. 클래서안 변수의 데이터 선언부분에
   class Player {
   String? name;
   Player({required this.name});
   }

   1. name parameter를 쓰기 위헤 {}를 사용한다.
   2. [required] 반드시 설정해줘야 하는 멤버임.
   3. 하지만 데이터명 뒤에 [?]를 써주는 경우는 name parameter를 설정해도 안해도 된다는 의미임.
